...
def gather_weight_data(wtype):
    ...
    for data in DataSet.read_from_file(WeightResult, WeightResult.get_path_to(wtype)):
        result = results.get(data.input().num_vertices)
        if result is None:
            result = ResultAccumulator(data.mst_weight)
            results[data.input().num_vertices] = result
        else:
            result.add_data(data.mst_weight)

    try:
        # open a file to output to
        fh = open(get_path_to_project_root() + 'writeup/data/weight/' + wtype, 'w')
        ...
        for num_verts in keys:
            r = results[num_verts]
            r.compute_stats()
            print >> fh, '%u %.3f %.3f %3.f' % (num_verts, r.lower99, r.mean, r.upper99)
        fh.close()
        return 0
    except IOError, e:
