...
plot fitdata using 1:3 title 'Data: random points in 2D-space' with linespoints, \
     const2fit(x)   title sprintf('%1.f: constant', const2fit_c)                                                               ls 1, \
     log2fit(x)     title sprintf('%.1f * log2(|V|) + %.1f: log', log2fit_m, log2fit_c)                                          ls 2, \
     lindlog2fit(x) title sprintf('%.1f * |V|^{%.1f} / log2(|V|) + %.1f: linear/log', lindlog2fit_m, lindlog2fit_e, lindlog2fit_c) ls 3, \
     lin2fit(x)     title sprintf('%.1f * |V|^{%.1f} + %.1f: linear', lin2fit_m, lin2fit_e, lin2fit_c)                           ls 4, \
     loglin2fit(x)  title sprintf('%.1f * |V|^{%.1f} * log2(|V|) + %.1f: linear*log', loglin2fit_m, loglin2fit_e, loglin2fit_c)    ls 5, \
